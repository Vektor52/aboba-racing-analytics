console.log("ACC car names loaded v8");
...
window.getCarName = function(carModel)
{
    if (carModel === null || carModel === undefined || carModel === "") {
        return "-";
    }

    const numericModel = Number(carModel);

    if (Number.isFinite(numericModel) && window.ACC_CARS[numericModel]) {
        return window.ACC_CARS[numericModel];
    }

    if (window.ACC_CARS[carModel]) {
        return window.ACC_CARS[carModel];
    }
